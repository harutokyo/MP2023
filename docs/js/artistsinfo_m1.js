const _m1data = [
    {
        base_id: "ando",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/ando.jpg",
        name: "安藤 美由",
        lab: "Miyu Ando",
        data: {
            name_ja: "安藤 美由｜Miyu Ando",
            name_en: "",
            name_lab: "服部研究室",
            profile_img: "./images/artistsinfo/m1/ando/1.jpg",
            work1_img: "./images/artistsinfo/m1/ando/1.jpg",
            work1_caption: "《確かさの体験》シングルチャンネルビデオ、2023",
            work2_img: "./images/artistsinfo/m1/ando/2.jpg",
            work2_caption: "《鶏皮飼育日記》写真と文章による記録行為、2019",
            info: `プロフィール
武蔵野美術大学造形学部映像学科卒業。
触る事で対象についての知見を得、その記録の集積を元に制作を行う。`,
            links: [
                {
                    text: "《鶏皮飼育日記》",
                    url: "https://note.com/nyun"
                }
            ]
        }
    },
    {
base_id: "izumi",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/izumi.jpg",
        name: "和泉 はるな",
        lab: "Haruna Izumi",
        data:
    {
    name_ja: "和泉 はるな｜Haruna Izumi",
            name_en: "",
            name_lab: "桐山研究室",
            work1_img: "./images/artistsinfo/m1/izumi/1.jpg",
            work1_caption: "《わたしはなになに》アニメーション、2023",
            work2_img: "./images/artistsinfo/m1/izumi/2.jpg",
            work2_caption: "《これはネコではない》インスタレーション、2019",
            work3_img: "./images/artistsinfo/m1/izumi/3.jpg",
            work3_caption: "《INVISIBLE》ポスター、2019",
            info: `プロフィール
1995年宮城県石巻市生まれ。
他者との論理的な共通認識としての作用を期待し、言語にアイデアを得て制作を行う。
多摩美術大学美術学部グラフィックデザイン学科卒業。

展示歴
2021「多摩美術大学グラフィックデザイン学科卒業制作展2021」（恵比寿ガーデンプレイス）
2020「多摩美術大学美術学部卒業制作展・大学院修了制作展B」（多摩美術大学）
2019「多摩美術大学地震ポスター支援プロジェクト・イラストレーションポスター展」（多摩美術大学）
2017「PORTFOLIO 2017」（6次元）
2016「JAGDA学生グランプリ2016」（東京ミッドタウン・デザインハブ）

受賞歴
2021「多摩美術大学卒業制作」優秀作品
2017「東京TDC賞2017」入選
2016「JAGDA学生グランプリ」入選

助成
2022 南カリフォルニア大学への派遣プログラム
`,
            links:
        [
                {
        text: "Twitter: @izumi_haruna9",
                    url: "https://twitter.com/izumi_haruna9"
                }
            ]
        }
},
    {
base_id: "itok",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/itok.jpg",
        name: "伊藤 琴音",
        lab: "Kotone Ito",
        data:
    {
    name_ja: "伊藤 琴音｜Kotone Ito",
            name_en: "",
            name_lab: "桂研究室",
            work1_img: "./images/artistsinfo/m1/itok/3.jpg",
            work1_caption: "《場景》サウンドインスタレーション、2023",
            work2_img: "./images/artistsinfo/m1/itok/2.jpg",
            work2_caption: "《あわいをきく》サウンドインスタレーション、2021",
            work3_img: "./images/artistsinfo/m1/itok/1.jpg",
            work3_caption: "《Selection》サウンドインスタレーション、2020",
            info: `プロフィール
1999年東京都生まれ。
2022年東京藝術大学音楽学部音楽環境創造科卒業。
聴覚を主軸とした知覚についての思索や制作を行う。
`,
            links:
        [
                {
        text: "Website",
                    url: "https://lit.link/KotoneIto"
                },
                {
        text: "Instagram: @k0t0ne0530",
                    url: "https://www.instagram.com/k0t0ne0530/"
                }
                //{
                    //text: "Mail: kotoneito530@gmail.com",
                    //url: "kotoneito530@gmail.com"
                //},
                ]
        }
},
    {
base_id: "itom",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/itom1.jpg",
        name: "伊藤 道史",
        lab: "Michibumi Ito",
        data:
    {
    name_ja: "伊藤 道史｜Michibumi Ito",
            name_en: "",
            name_lab: "桂研究室",
            work1_img: "./images/artistsinfo/m1/itom/1.jpg",
            work1_caption: "《PHOTOPHOBIA1》《PHOTOPHOBIA2》《PHOTOPHOBIA3》VR、2023",
            work2_img: "./images/artistsinfo/m1/itom/2.jpg",
            work2_caption: "《クロールする蛹のためのレクチャー》インスタレーション、2021",
            work3_img: "./images/artistsinfo/m1/itom/3.jpg",
            work3_caption: "《 皮膚祭 / XipeTotec Reality》VR、2021",
            info: `プロフィール
バーチャルリアリティを中心に用いて制作をしてきた。
病理や宗教などある人々にとって非現実的(バーチャル / フィクショナル)なイメージを扱いながら、今私たちがいる時空間とは別にありえた世界について考えている。

展示歴
2022 個展「私であることの試み / CONATUS」（日本橋 NEORT++）
2021 個展「クロールする蛹のためのレクチャー」 （TIME MACHINE）
2022「NEWVIEW FEST 2021 - 皮膚祭 / XipeTotec Reality」（渋谷PARCO 4F / バーチャル会場）
2021「IAMAS_Open_house 2021」（オンライン）
`,
            links:
        [
                {
        text: "Twitter: @mi_mi_mi_mi65",
                    url: "https://twitter.com/mi_mi_mi_mi65"
                },
                {
        text: "Instagram: @michibumiito",
                    url: "https://www.instagram.com/michibumiito/"
                }
            ]
        }
},
    {
base_id: "eguchi",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/eguchi.jpg",
        name: "江口 智之",
        lab: "Tomoyuki Eguchi",
        data:
    {
            name_ja: "江口 智之｜Tomoyuki Eguchi",
            name_en: "",
            name_lab: "桂研究室",
            work1_img: "./images/artistsinfo/m1/eguchi/1.jpg",
            work1_caption: "《Mariage》シングルチャンネルビデオ、2022-2023",
            work2_img: "./images/artistsinfo/m1/eguchi/2.jpeg",
            work2_caption: "《Nastagio degli Onesti》ループムービー・インスタレーション、2022",
            work3_img: "./images/artistsinfo/m1/eguchi/3.jpg",
            work3_caption: "《PERFECT CRIME》ループムービー・インスタレーション、2021",
            info: `プロフィール
1998年生まれ。2022年多摩美術大学絵画学科油画専攻卒業。
美術館をバックグラウンドに持つ意識から美術の特権性及び、公益性についての研究、展示を行う。

展示歴
2022「ストーンテープ~見たら呪われる展示~」（PARA）
2021「Navigated Locomotion」（White Gallery）
2021「獸(第 0 章 / 交叉時点)」（BUoY）
2020 個展「スピーカー」（rusu）

活動歴
2020「ウォーターフォールを追いかけて」映像制作（早稲田小劇場どらま館招聘公演・オンライン上演）
`,
            links:
        [
                {
        text: "Twitter: @tomoyukieguchi",
                    url: "https://twitter.com/tomoyukieguchi"
                },
                {
        text: "Instagram: @f7dob6",
                    url: "https://www.instagram.com/f7d0b6/"
                }
            ]
        }
},
    {
base_id: "oda",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/oda.jpg",
        name: "小田 陽菜乃",
        lab: "Hinano Oda",
        data:
    {
    name_ja: "小田 陽菜乃｜Hinano Oda",
            name_en: "",
            name_lab: "畠山研究室",
            work1_img: "./images/artistsinfo/m1/oda/1.jpg",
            work1_caption: "《I see.》シングルチャンネルビデオ、2023",
            work2_img: "./images/artistsinfo/m1/oda/2.jpg",
            work2_caption: "《食い入るように》インスタレーション、2021",
            work3_img: "./images/artistsinfo/m1/oda/3.jpg",
            work3_caption: "《丁寧な錯覚》インスタレーション、2020",
            info: `プロフィール
1997年神奈川県生まれ。
2022年金沢美術工芸大学美術工芸学部美術科彫刻専攻卒業。
作品を多視点から鑑賞する際の身体感覚を通して、見ているものの曖昧さについて考え、制作を行う。

展示歴
2021「ストレンジャーによろしく」（箔一ビル）
2019「金沢彫刻祭2019」（じょーの箱）
2019「oterart2019」（崇禅寺）
2019「東京インディペンデント」（東京藝術大学美術館 陳列館）
2019 二人展「胎児の夢」（DESIGN FESTA GALLERY WEST1-D）

活動歴
金沢美術工芸大学アートプロジェクトチーム「スズプロ」で活動。
2021「奥能登国際芸術祭2020+」（旧八木家）

助成
2021 三菱商事アート・ゲート・プログラム
`,
            links:
        [
                {
        text: "Twitter: @taiyooo1130",
                    url: "https://twitter.com/taiyooo1130"
                },
                {
        text: "Instagram: @taiyooo1130",
                    url: "https://www.instagram.com/taiyooo1130/"
                }
            ]
        }
},
    {
base_id: "kurachi",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/kurachi.jpg",
        name: "倉知 朋之介",
        lab: "Tomonosuke Kurachi",
        data:
    {
    name_ja: "倉知 朋之介｜Tomonosuke Kurachi",
            name_en: "",
            name_lab: "桂研究室",
            work1_img: "./images/artistsinfo/m1/kurachi/1.jpg",
            work1_caption: "《PoPoPot》映像インスタレーション、2023",
            work2_img: "./images/artistsinfo/m1/kurachi/2.jpg",
            work2_caption: "《ムシ図鑑》インスタレーション・映像、2022",
            work3_img: "./images/artistsinfo/m1/kurachi/3.jpg",
            work3_caption: "《チョコチップクッキー＆ミルク》インスタレーション・映像、2022",
            info: `プロフィール
1997年愛知県生まれ。
2020年京都造形芸術大学（現 京都芸術大学）情報デザイン学科卒業。
日常生活のなかにある「可笑しさ」を帯びた存在や状況に焦点を当て、映像作品を中心に制作を行う。

展示歴
2022「惑星ザムザ」（小高製本工業株式会社跡地）
2022「ARTISTS’FAIR KYOTO 2022」（京都新聞ビル 地下1階）
2022「獸（第1章 / 宝町団地）」（CONTRAST）
2022「P.O.N.D.」 （渋谷PARCO）
など

受賞歴
2020「2019年度京都造形芸術大学作品展」優秀賞
`,
            links:
        [
                {
        text: "Twitter: @kadojr91",
                    url: "https://twitter.com/kadojr91"
                },
                {
        text: "Instagram: @kadojr91",
                    url: "https://www.instagram.com/kadojr91/"
                }
            ]
        }
},
    {
base_id: "korenaga",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/korenaga.jpg",
        name: "是永 悠之介",
        lab: "Yunosuke Korenaga",
        data:
    {
    name_ja: "是永 悠之介｜Yunosuke Korenaga",
            name_en: "",
            name_lab: "高山研究室",
            work1_img: "./images/artistsinfo/m1/korenaga/1.jpg",
            work1_caption: "《影を見て歩く向きを決める》チャンネルビデオ、2023",
            work2_img: "./images/artistsinfo/m1/korenaga/2.jpg",
            work2_caption: "《空ける、ユーリカ》インスタレーション・映像、2022",
            work3_img: "./images/artistsinfo/m1/korenaga/3.jpg",
            work3_caption: "《Look out of the window》シングルチャンネルビデオ、2020",
            info: `プロフィール
2000年大分県生まれ。
東京造形大学造形学部美術学科絵画専攻卒業。
インスタレーション、映像を中心に制作。

展示歴
2022「ZOKEI展」（東京造形大学）
`,
            links:
        [
                {
        text: "Twitter: @aulcb",
                    url: "https://twitter.com/aulcb/"
                },
                {
        text: "Instagram: @korenaga.jp",
                    url: "https://www.instagram.com/korenaga.jp/"
                }
            ]
        }
},
    {
base_id: "saito",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/saito.jpg",
        name: "齋藤 千春",
        lab: "Chiharu Saito",
        name_lab: "畠山研究室",
        data:
    {
    name_ja: "齋藤 千春｜Chiharu Saito",
            name_en: "",
            work1_img: "./images/artistsinfo/m1/saito/1.jpeg",
            work1_caption: "《TIME STUDY》インスタレーション、2023",
            work2_img: "./images/artistsinfo/m1/saito/2.jpg",
            work2_caption: "《Somewhere》写真、2021",
            //work3_img: "./images/artistsinfo/m1/saito/3.jpg",
            //work3_caption: "hogehoge",
            info:`プロフィール
2000年生まれ。
日本大学芸術学部写真学科卒業。
`,
            links:
        [
                {
        text: "Website",
                    url: "https://www.chiharusaito.net"
                },
            ]
        }
},
    {
base_id: "suzuki",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/suzuki.jpg",
        name: "鈴木 創大",
        lab: "Sota Suzuki",
        data:
    {
    name_ja: "鈴木 創大｜Sota Suzuki",
            name_en: "",
            name_lab: "畠山研究室",
            work1_img: "./images/artistsinfo/m1/suzuki/1.jpg",
            work1_caption: "《Gate》ビデオインスタレーション、2023",
            work2_img: "./images/artistsinfo/m1/suzuki/2.jpg",
            work2_caption: "《Delicious water》インスタレーション・映像、2022",
            work3_img: "./images/artistsinfo/m1/suzuki/3.jpg",
            work3_caption: "《139.72646653207784° 35.665113849127884°》インスタレーション、2021",
            info: `プロフィール
1999年生まれ。
2022年多摩美術大学美術学部絵画学科油画専攻卒業。
近年は『ペラい場所性』についてをテーマにインスタレーションや映像などを中心に制作をしている。
またVJ・映像作家としても活動している。

展示歴
2022「ART AWARD TOKYO MARUNOUCHI2022」（丸ビルマルキューブ）
2022「多摩美術大学 卒業制作展・大学院修了制作展」
2022「第45回 東京五美術大学連合卒業・修了制作展」
2021 個展「Delicious Water」(TAU Painting East Wing 402)
2020「フラット３階建て」（プライベイト）
2019「acousticlubvol.4」（千代田区 内幸町ホール）

受賞歴
2022「ART AWARD TOKYO MARUNOUCH2022」今村有策賞
2022「多摩美術大学 卒業制作展」優秀賞
`,
            links: [
                {
                    text: "Website",
                    url: "https://site-2251842-4277-9677.mystrikingly.com/"
                },
                {
                    text: "Twitter: @sota_suzu",
                    url: "https://twitter.com/sota_suzu"
                },
                {
                    text: "Instagram: @sota.s",
                    url: "https://www.instagram.com/sota.s/"
                }
            ]
        }
    },
    {
        base_id: "chanya",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/chanya.jpg",
        name: "チャンヤ･スィウランポン",
        lab: "Chanya Sriuranpong",
        data: {
            name_ja: "チャンヤ･スィウランポン｜Chanya Sriuranpong",
            name_en: "",
            name_lab:"桐山研究室",
            work1_img: "./images/artistsinfo/m1/chanya/2.jpg",
            work1_caption: "《DUCK!》インタラクティブゲーム、2023",
            work2_img: "./images/artistsinfo/m1/chanya/1.png",
            work2_caption: "《GOOD MORNING》インタラクティブアニメーション、2022",
            info: `プロフィール
タイ、バンコク出身。
チュラーロンコーン大学コミュニケーションデザイン学科卒業。

展示歴
（個人）
2019「CommDe Creative Walk 2019 'Commbo'」（卒業・修了制作展）
（グループ・企画）
2017「SILENT SPEAKER」（RATCHADAMNONE CONTENPORARY ART CENTRE (RCAC)）
2017「INTERNATIONAL COLLABORATION OF CHULALONGKORN UNIVERSITY AND SHIH CHIEN UNIVERSITY TAIPEI」

受賞歴
2018「ART GAMES – AN INTERNATIONAL GAME JAM NETWORK 」（Top 3 Finalist）
`,
            links: [
                {
                    text: "Website",
                    url: "https://www.chaatenn.com"
                },
            ]
        }
    },
    {
        base_id: "homma",
        thumb_img_path: "./images/artistsinfo/m1/thumbnails/homma.jpg",
        name: "本間 悠暉",
        lab: "Yuki Homma",
        data: {
            name_ja: "本間 悠暉｜Yuki Homma",
            //name_en: "Yuki Homma",
            name_lab:"桂研究室",
            work1_img: "./images/artistsinfo/m1/homma/3.jpg",
            work1_caption: "《Chronicle of a void》シングルチャンネルビデオ、2023",
            work2_img: "./images/artistsinfo/m1/homma/1.jpg",
            work2_caption: "《Para-Vision》VR、2022",
            //work3_img: "./images/artistsinfo/m1/homma/2.jpg",
            //work3_caption: "《Project GIBSON（グループ・企画 2021）》",
            info: `プロフィール
神奈川県横浜市出身。
東京大学経済学部経済学科卒業。
都市やインターネット上に潜む不安定な場所、そこで生起される関係性を記録し、虚実入り混じる物語を構築する。
MESON,inc.にてAR/VR技術を活用した体験の研究活動に参画。


展示歴
（グループ・企画）
2021「SIGGRAPH ASIA 2021」（東京国際フォーラム）
2020「未来の学校祭」（東京ミッドタウン）
2020 「CES2020」（ラスベガス・コンベンションセンター）

活動歴
2021 国土交通省Project Plateau 民間サービス実証調査参画
2020 ノイタミナ系列『富豪刑事』ARデザインアドバイザー参画

受賞歴
2021「SIGGRAPH ASIA2021 XR」論文掲載

刊行物
2021 『GIBSON: AR/VR synchronized city walking system』（SIGGRAPH ASIA 2021 XR）論文共著
2021 『WIRED JAPAN VOL.41』プロダクト掲載
`,
            links: [
                {
                    text: "Twitter: @y__homm",
                    url: "https://twitter.com/y__homm"
                },
                {
                    text: "Instagram: @hommynm",
                    url: "https://www.instagram.com/HommyNM/"
                }
            ]
        }
    },
]

